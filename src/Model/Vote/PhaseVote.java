...
public class PhaseVote {
    ...
    public void demarrerVote(GestionJoueur gestionJoueur,
        ...
        while (!phaseTerminee) {
            ...
            Joueur cible = choisirCiblePourVote(joueur, gestionJoueur.getListeJoueurs());
              if (cible != null) {
                  joueur.voter(cible); // ✅ Utilise la méthode voter() qui peut être décorée
              }            }
            System.out.println("\n-------------- VOTE TERMINÉ --------------\n");
        ...
        private Joueur choisirCiblePourVote(Joueur votant, List<Joueur> joueurs) {
        System.out.println("\n" + votant.getNom() + ", choisissez un joueur à voter :");

        // Afficher la liste des joueurs (sauf soi-même)
        List<Joueur> ciblesPossibles = joueurs.stream()
                .filter(j -> !j.equals(votant) && j.isVivant())
                .collect(Collectors.toList());

        for (int i = 0; i < ciblesPossibles.size(); i++) {
            System.out.println((i + 1) + " - " + ciblesPossibles.get(i).getNom());
        }

        Scanner sc = new Scanner(System.in);
        try {
            int choix = sc.nextInt();
            if (choix >= 1 && choix <= ciblesPossibles.size()) {
                return ciblesPossibles.get(choix - 1);
            }
        } catch (Exception e) {
